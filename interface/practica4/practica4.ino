...
//const char* ssid = "MEGACABLE-C9A2";       // Reemplaza con tu SSID
        ...
        if (len > 0) {
            ...
            if (!error) {
                ...
                if (strcmp(tipo, "heartbeat") == 0) {
                  ...
                  // Nuevo comando para evaluar distancias con el servomotor
                  else if (comando == 8) {
                    Serial.println("Comando recibido: evaluar distancias");
                    int posicion = 0;    
                    miServo.write(posicion);  // Mover servomotor a la posición
                    //delay(500);                   // Esperar para que el servo se estabilice
                    float distancia = medicion();
                    DynamicJsonDocument doc(256);
                    doc["distancia"] = distancia;
                    // Serializar el JSON y enviarlo
